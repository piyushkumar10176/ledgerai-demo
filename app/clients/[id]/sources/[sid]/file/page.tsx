...
export default async function FilePage({
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      <Link href={`/clients/${clientId}/sources/${source.id}`} className="text-sm text-stone-500 hover:text-stone-800">← {source.business_name}</Link>
      ...
      <div className="mt-2 inline-flex items-center gap-2 rounded-md bg-stone-100 px-3 py-1 text-xs text-stone-600">
      ...
      <div className="mt-4 flex items-center justify-between rounded-xl border border-brand-200 bg-brand-50 px-5 py-3">
        <span className="font-semibold text-brand-900">Net profit (YTD)</span>
        <span className="text-xl font-bold tabular-nums text-brand-900">{formatGBP(u.netProfit)}</span>
      </div>

      <div className="mt-3 flex items-center justify-between rounded-xl border border-stone-200 bg-white px-5 py-3">
        <div>
          ...
          <div className="text-xs text-stone-400">{tax.basis}</div>
        </div>
...
function LineRow({ label, amount }: { label: string; amount: number }) {
  return <div className="flex justify-between border-t border-stone-100 py-1"><span>{label}</span><span className="tabular-nums">{formatGBP(amount)}</span></div>;
}
function Total({ label, amount }: { label: string; amount: number }) {
  return <div className="mt-1 flex justify-between border-t-2 border-stone-300 py-1 font-semibold"><span>{label}</span><span className="tabular-nums">{formatGBP(amount)}</span></div>;
}
function Empty() { return <div className="py-1 text-stone-400">None</div>; }
