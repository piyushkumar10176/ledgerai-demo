...
export default function ControlTower({
  ...
  return (
    <div>
      <div className={"flex flex-wrap items-center justify-between gap-2 rounded-xl border px-5 py-3 " + (soon ? "border-amber-300 bg-amber-50" : "border-slate-200 bg-white")}>
        <div>
          <div className="text-sm font-semibold">
            {period.label} · MTD Income Tax {period.taxYear} · cumulative to {period.end}
          </div>
          <div className="text-xs text-slate-500">Quarterly deadline {period.due}</div>
        </div>
        <div className="flex items-center gap-3">
          <div className={"rounded-full px-3 py-1 text-sm font-medium " + (soon ? "bg-amber-200 text-amber-900" : "bg-slate-100 text-slate-700")}>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {(["all", "missing", "ready", "filed"] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={"rounded-full px-3 py-1 text-sm " + (filter === f ? "bg-slate-900 text-white" : "border border-slate-200 text-slate-600 hover:bg-slate-50")}>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((o) => {
              ...
              return (
                <tr key={o.sourceId} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={"rounded-full px-2 py-0.5 text-xs " + (o.mandationStatus === "mandated" ? "bg-slate-100 text-slate-700" : "bg-slate-50 text-slate-400")}>
                  ...
                  <td className="px-4 py-3">
                    ...
                    {o.status === "filed" && o.filedRef && (
                      <div className="mt-1 font-mono text-[10px] text-slate-400">{o.filedRef}</div>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    {o.status === "missing" ? (
                      ...
                      <Link href={`/clients/${o.clientId}/sources/${o.sourceId}/file`}
                        className="rounded-md bg-indigo-600 px-3 py-1 text-xs font-medium text-white hover:bg-indigo-700">Review &amp; file →</Link>
                    ) : (
                      <Link href={`/clients/${o.clientId}/sources/${o.sourceId}/file`} className="text-xs text-slate-500 hover:text-slate-800">Open</Link>
                    )}
                  </td>
                </tr>
              );
            ...
            {rows.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-400">No obligations in this view.</td></tr>
            )}
          </tbody>
        </table>
      </div>
